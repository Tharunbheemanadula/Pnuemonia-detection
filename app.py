...
def home():
    if request.method=='POST':
        if 'img' not in request.files:
            return render_template('home.html',filename="illuminators.jpeg",message="Please upload an file")
        f = request.files['img']  
        if f.filename=='':
            return render_template('home.html',filename="illuminators.jpeg",message="No file selected")
        if not ('jpeg' in f.filename or 'png' in f.filename or 'jpg' in f.filename):
            return render_template('home.html',filename="illuminators.jpeg",message="Please upload an image with .png or .jpg/.jpeg extension")
        files = os.listdir(app.config['UPLOAD_FOLDER'])
        if len(files)==1:
            f.save(os.path.join(app.config['UPLOAD_FOLDER'],f.filename))
        else:
            files.remove("illuminators.jpeg")
            file_ = files[0]
    ...
    return render_template('home.html',filename='illuminators.jpeg')
